...
type DragAction = 
  | "sel-left" | "sel-right" | "sel-move" 
  | "zoom-left" | "zoom-right" | "zoom-move" 
  | null;
...
export default function AudioTimeSelector({
  ...
  const dragStartRef = useRef<{ 
    ...
    startVal: number; 
    endVal: number;
    viewStartVal: number; // Added to prevent relying on state in dragging loop
    viewEndVal: number;   // Added
    durationVal: number;  // Added
    containerWidth: number;
  }>({ mouseX: 0, startVal: 0, endVal: 0, viewStartVal: 0, viewEndVal: 1, durationVal: 1, containerWidth: 0 });

  // Maintain reference to the latest onChange so we don't need it in useEffect deps
    ...
    const processAudio = async () => {
      ...
      try {
        ...
        if (initialStart === 0 && initialEnd === 0) {
          setStart(fileDuration * 0.1);
          setEnd(fileDuration * 0.9);
        } else {
  ...
  }, [audioBlob]); // REMOVED initialStart & initialEnd to avoid zooming out on parent re-renders
  ...
  const handleMouseDown = useCallback((e: React.MouseEvent, type: DragAction, containerRef: React.RefObject<HTMLDivElement>) => {
    ...
    dragStartRef.current = {
      mouseX: e.clientX,
      startVal: type?.startsWith('sel') ? start : viewStart,
      endVal: type?.startsWith('sel') ? end : viewEnd,
      viewStartVal: viewStart,
      viewEndVal: viewEnd,
      durationVal: duration,
      containerWidth: rect.width,
    };
  }, [start, end, viewStart, viewEnd, duration]);
  ...
  useEffect(() => {
    ...
    const handleMouseMove = (e: MouseEvent) => {
      // Use locked ref values rather than rapidly changing state values
      const { mouseX, startVal, endVal, viewStartVal, viewEndVal, durationVal, containerWidth } = dragStartRef.current;
      ...
      if (dragging.startsWith('sel')) {
        ...
        if (dragging === "zoom-left") {
          newViewStart = Math.max(0, Math.min(startVal + deltaSeconds, viewEndVal - minZoomGap));
          setViewStart(newViewStart);
        } else if (dragging === "zoom-right") {
          newViewEnd = Math.min(durationVal, Math.max(endVal + deltaSeconds, viewStartVal + minZoomGap));
          setViewEnd(newViewEnd);
        } else if (dragging === "zoom-move") {
          const span = endVal - startVal;
          newViewStart = startVal + deltaSeconds;
  ...
  }, [dragging]); // Safe dependency limit keeps events bound smoothly and cleanly through the whole drag 
    ...
    <div className="w-full font-sans flex flex-col gap-[10px]">
      ...
      >
        ...
        {isSelectionVisible && (
          ...
          >
            <div
              className={`absolute left-[-6px] top-0 bottom-0 w-[12px] cursor-col-resize flex justify-center items-center group z-20 ${start < viewStart ? 'hidden' : ''}`}
            ...
            <div
              className={`absolute right-[-6px] top-0 bottom-0 w-[12px] cursor-col-resize flex justify-center items-center group z-20 ${end > viewEnd ? 'hidden' : ''}`}
